...
class PositionalEncodings(nn.Module):
    ...
    def forward(self, E_idx):
        ...
        E = torch.cat((torch.cos(angles).cpu(), torch.sin(angles).cpu()), -1)
        return E
...
class ProteinFeatures(nn.Module):
    ...
    def forward(self, X, L, mask):
        ...
        #print('RBF', RBF.device)
        #print(' E_positional',  E_positional.device)
       ...
       # E = self.norm_edges(E)
       # V = self.node_embedding(V.to(torch.float32))
       # V = self.norm_nodes(V.to(torch.float32))
        E = self.edge_embedding(E.to(torch.float32))
        E = self.norm_edges(E.to(torch.float32))
