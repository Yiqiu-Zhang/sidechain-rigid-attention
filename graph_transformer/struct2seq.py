...
class Struct2Seq(nn.Module):
    def __init__(self, node_features=384, edge_features=384,
        hidden_dim=384, num_encoder_layers=6, num_decoder_layers=0,
    ...
    def forward(self, Angle_embedding, X, mask, L=100):
        """ Graph-conditioned sequence model
            YingLv: L is not used in function features. Thus, we set the L=100
        """

        # Prepare node and edge embeddings
        E, E_idx = self.features(X, L, mask)
        h_V = self.W_v(Angle_embedding)
        h_E = self.W_e(E)
