...
class InputEmbedder(nn.Module):
    def __init__(
        ...
        self.linear_relpos = nn.Linear(self.no_bins +10, c_z)
    ...
    def forward(self,
        ...
        pair_emb = self.pair_embedder(nf_pair_emb, relative_pos, pair_mask)
...
class EdgeInvariantPointAttention(nn.Module):
    ...
    def forward(self,
        ...
        square_mask_e = self.inf * (pair_mask_e - 1)  # 这里靠 mask 逼近 -inf 之后再用 softmax 让 attention score 变 0
        ...
        s = self.linear_out(
            torch.cat(
                (o, *torch.unbind(o_pt, dim=-1), o_pt_norm, o_pair), dim=-1 # o_pair
            )).float()
...
class StructureUpdateModule(nn.Module):
    ...
    def forward(self,
        ...
        for i, block in enumerate(self.blocks):
            ...
            delta_chi, unnormalized_delta_chi = self.angle_resnet(node_emb)

            # update global rigids and local rigids
            rigids, initial_r = structure_build.torsion_to_frame(seq_idx, bb_to_gb, delta_chi, initial_r)
        ...
        return rigids, unnormalized_delta_chi
...
class StructureBlock(nn.Module):
    ...
    def forward(self,
        ...
        ipa_emb = self.edge_ipa(node_emb, pair_emb, rigids, pair_mask, E_idx)
...
class RigidDiffusion(nn.Module):
    ...
    def forward(self,
        ...
        # [*, N_rigid] Rigid
        initial_rigids = structure_build.torsion_to_frame(seq_idx,
                                                 bb_to_gb,
                                                 angles_sin_cos,
                                                 default_r)  # add attention #frame

        # flat_mask [*, N_rigid]， others [*, N_rigid, N_rigid, c]
        pair_mask, rigid_mask, distance, altered_direction, orientation = structure_build.frame_to_edge(initial_rigids, seq_idx, pad_mask)

        E_idx = structure_build.update_E_idx(initial_rigids, pair_mask, self.top_k)

        # [*, N_rigid, c_n], [*, N_rigid, N_rigid, c_z]
        node_emb, pair_emb = self.input_embedder(
                                                  seq_esm,
                                                  rigid_type,
                                                  rigid_property,
                                                #  distance,
        ...
        # [*, N_res, c_n * 5]      
        delta_chi, unnormalized_delta_chi = self.structure_update(seq_idx,
                                        bb_to_gb,
                                        node_emb,
                                        pair_emb,
                                        initial_rigids,
        ...
        return pred_chi_sin_cos, unnormalized_delta_chi
